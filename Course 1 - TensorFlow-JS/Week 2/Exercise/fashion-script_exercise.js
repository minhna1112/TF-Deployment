...
function getModel() {
    ...
    // YOUR CODE HERE
	model.add(tf.layers.conv2d({inputShape: [28,28,1], activation: 'relu', kernelSize: 5, filters: 8, padding: 'same'}));
	model.add(tf.layers.conv2d({activation: 'relu', kernelSize: 3, filters: 8, padding: 'same'}));
	model.add(tf.layers.maxPooling2d({poolSize: [2,2]}));
	
	model.add(tf.layers.conv2d({activation: 'relu', kernelSize: 3, filters: 16, padding: 'same'}));
	model.add(tf.layers.conv2d({activation: 'relu', kernelSize: 2, filters: 16, padding: 'same'}));
	model.add(tf.layers.maxPooling2d({poolSize: [2,2]}));
	
	model.add(tf.layers.conv2d({activation: 'relu', kernelSize: 2, filters: 32, padding: 'same'}));
	model.add(tf.layers.maxPooling2d({poolSize: [2,2]}));
	
	model.add(tf.layers.conv2d({activation: 'relu', kernelSize: 2, filters: 64, padding: 'same'}));
	model.add(tf.layers.maxPooling2d({poolSize: [2,2]}));
    ...
    model.compile({optimizer: tf.train.adam(), metrics: ['accuracy'], loss: 'categoricalCrossentropy'});
